...
# Launch a GPU interactive pod (A100 80GB) and exec into it.
# Repo is cloned and aurora is pre-installed. Working dir: /opt/repo
# Cleans up the pod when you exit the shell.

POD_NAME="aurora-dev"
YAML="k8s/aurora-interactive-pod.yaml"

kubectl delete pod "$POD_NAME" --ignore-not-found=true 2>/dev/null

echo "Creating pod: $POD_NAME (A100 80GB)"
kubectl create -f "$YAML"

echo "Waiting for pod to be scheduled (GPU pods may take a few minutes)..."
